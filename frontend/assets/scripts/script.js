...
/** Icon for the 'light' theme */
const elmThemeIconLight = document.querySelector('#theme-icon-light');

/** Icon for the 'dark' theme */
const elmThemeIconDark = document.querySelector('#theme-icon-dark');

/** Icon for the 'random' theme */
const elmThemeIconRandom = document.querySelector('#theme-icon-random');

/** Accessibility element to announce theme changes */
const elmThemeStatus = document.querySelector('#theme-status');
...
/**
 * Updates the theme icon and accessibility status message.
 * @param {string} theme - The theme to activate ('light', 'dark', 'random').
 */
function updateThemeUI(theme) {
    // Ensure all icons are valid elements before proceeding
    if (!elmThemeIconLight || !elmThemeIconDark || !elmThemeIconRandom) return;

    // Hide all icons by default
    elmThemeIconLight.classList.add('hidden');
    elmThemeIconDark.classList.add('hidden');
    elmThemeIconRandom.classList.add('hidden');

    // Show the correct icon based on the theme
    if (theme === 'light') {
        elmThemeIconLight.classList.remove('hidden');
    } else if (theme === 'dark') {
        elmThemeIconDark.classList.remove('hidden');
    } else if (theme === 'random') {
        elmThemeIconRandom.classList.remove('hidden');
    }

    // Announce the change for screen readers
    if (elmThemeStatus) {
        elmThemeStatus.textContent = `Theme changed to ${theme}.`;
    }
}
...
/** Update the icon to match the current theme on page load */
...
        localStorage.setItem('theme', currentTheme);
        ...
        // Call our new functions to update the UI
        updateThemeUI(currentTheme);
